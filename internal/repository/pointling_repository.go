...
type API interface {
	...
	// GetUserPointHistory retrieves point spend history for a user
	GetUserPointHistory(userID int64, limit, offset int) ([]*models.PointSpend, error)
}
...
func (r *PointlingRepository) GetUserPointHistory(userID int64, limit, offset int) ([]*models.PointSpend, error) {
	query := `
		SELECT user_id, item_id, points, created_at
		FROM public.point_spend
		WHERE user_id = $1
		ORDER BY created_at DESC
		LIMIT $2 OFFSET $3`
	...
	if err != nil {
		return nil, fmt.Errorf("get user point history query: %w", err)
	}
	defer rows.Close()

	var history []*models.PointSpend
	for rows.Next() {
		entry := &models.PointSpend{}
		err := rows.Scan(
			&entry.UserID,
			&entry.ItemID,
			&entry.PointsSpent,
		...
		if err != nil {
			return nil, fmt.Errorf("scan point spend row: %w", err)
		}
		history = append(history, entry)
	}
	if err = rows.Err(); err != nil {
		return nil, fmt.Errorf("iterate point spend rows: %w", err)
	}
	return history, nil
}
